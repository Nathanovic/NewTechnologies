...
Properties
	...
	SubShader
		...
		Pass
		{
			...
			float _xDistortionOne;
			float _xDistortionTwo;
			float _yDistortionOne;
			float _yDistortionTwo;
			...
			v2f vert(appdata v)
			{
				...
				o.pos.x *= lerp(sin(_Distortion * _time), _xDistortionOne, _xDistortionTwo);
				o.pos.y *= lerp(sin(-_Distortion * _time), _yDistortionOne, _yDistortionTwo);
			...
			{
				float4 texcol = tex2D(_MainTex, i.uv * _Distortion) ;
